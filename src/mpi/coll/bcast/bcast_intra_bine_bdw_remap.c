                                    ...
                                    MPIR_Comm *comm_ptr, int coll_attr)
{
    ...
    while (mask < comm_size) {
        ...
        if (recvd) {
            if (!is_contig) {
                mpi_errno = MPIC_Send((char *) tmp_buf + displs[send_block_first] * type_size,
                                      send_count * type_size, MPIR_BYTE_INTERNAL, abs_partner,
                                      MPIR_BCAST_TAG, comm_ptr, coll_attr);
            } else {
                mpi_errno = MPIC_Send((char *) buf + displs[send_block_first] * extent, send_count,
                                      datatype, abs_partner, MPIR_BCAST_TAG, comm_ptr, coll_attr);
            }
            MPIR_ERR_CHECK(mpi_errno);
        } else if (inverse_mask == receiving_mask || partner == 0) {
            if (!is_contig) {
                mpi_errno = MPIC_Recv((char *) tmp_buf + displs[recv_block_first] * type_size,
                                      recv_count * type_size, MPIR_BYTE_INTERNAL, abs_partner,
                                      MPIR_BCAST_TAG, comm_ptr, MPI_STATUS_IGNORE);
            } else {
                mpi_errno = MPIC_Recv((char *) buf + displs[recv_block_first] * extent, recv_count,
    ...
    while (mask > 0) {
        ...
        if (!is_contig) {
            mpi_errno = MPIC_Sendrecv((char *) tmp_buf + displs[send_block_first] * type_size,
                                      send_count * type_size, MPIR_BYTE_INTERNAL, abs_spartner,
                                      MPIR_BCAST_TAG,
                                      (char *) tmp_buf + displs[recv_block_first] * type_size,
                                      recv_count * type_size, MPIR_BYTE_INTERNAL, abs_rpartner,
                                      MPIR_BCAST_TAG, comm_ptr, MPI_STATUS_IGNORE, coll_attr);
        } else {
            mpi_errno = MPIC_Sendrecv((char *) buf + displs[send_block_first] * extent, send_count,
                                      datatype, abs_spartner, MPIR_BCAST_TAG,
                                      (char *) buf + displs[recv_block_first] * extent, recv_count,
                                      datatype, abs_rpartner, MPIR_BCAST_TAG, comm_ptr, MPI_STATUS_IGNORE,
                                      coll_attr);
        }
        MPIR_ERR_CHECK(mpi_errno);
