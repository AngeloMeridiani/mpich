...
{
    int comm_size, rank, stsize, rtsize, mpi_errno = MPI_SUCCESS;
    ...
    mask = 0x1 << (int) (MPII_Bine_log2(comm_size) - 1);
    sbuf_offset = rank;
    if (root == rank) {
        recvd = 1;
        sbuf = (char *) sendbuf;
    ...
    while (mask > 0) {
        ...
        if (recvd) {
            ...
            if (recv_start == recv_end) {
                ...
                MPIR_CHKLMEM_MALLOC(tmp_buf, recvcount * num_blocks * stsize);
                sbuf = (char *) tmp_buf;
                rbuf = (char *) tmp_buf;
            ...
            if (recv_end >= recv_start) {
                if (rbuf == (char *) tmp_buf) {
                    mpi_errno =
                        MPIC_Recv((char *) rbuf, recvcount * num_blocks * rtsize,
                ...
                if (rbuf == (char *) tmp_buf) {
                    mpi_errno =
                        MPIC_Recv((char *) rbuf,
                                  recvcount * ((comm_size - 1) - recv_start + 1) * rtsize,
                                  MPIR_BYTE_INTERNAL, partner, MPIR_SCATTER_TAG, comm_ptr,
                                  MPI_STATUS_IGNORE);
                    MPIR_ERR_CHECK(mpi_errno);
                    mpi_errno =
                        MPIC_Recv((char *) rbuf +
                                  (recvcount * ((comm_size - 1) - recv_start + 1)) * stsize,
                                  recvcount * (recv_end + 1) * rtsize, MPIR_BYTE_INTERNAL, partner,
    ...
    if (!is_leaf && (recvbuf != MPI_IN_PLACE)) {
        mpi_errno =
            MPIR_Localcopy((char *) sbuf + sbuf_offset * recvcount * stsize, recvcount * stsize,
                           MPIR_BYTE_INTERNAL, (char *) recvbuf, recvcount * rtsize,
                           MPIR_BYTE_INTERNAL);
        MPIR_ERR_CHECK(mpi_errno);
    }
