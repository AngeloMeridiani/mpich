                               ...
                               MPI_Op op, int root, MPIR_Comm * comm_ptr, int coll_attr)
{
    ...
    int comm_size, rank, vrank, adjsize, dtsize, mpi_errno = MPI_SUCCESS, steps, step;
    ...
    if (vrank < (2 * extra_ranks)) {
        if (0 != (vrank % 2)) {
            mpi_errno = MPIC_Send(resbuf, count, datatype, MPII_Bine_mod((vrank - 1) + root, comm_size),
                                  MPIR_REDUCE_TAG, comm_ptr, coll_attr);
            MPIR_ERR_CHECK(mpi_errno);
            loop_flag = 1;
        } else {
            mpi_errno = MPIC_Recv(tmpbuf, count, datatype, MPII_Bine_mod((vrank + 1) + root, comm_size),
                                  MPIR_REDUCE_TAG, comm_ptr, MPI_STATUS_IGNORE);
            MPIR_ERR_CHECK(mpi_errno);
            mpi_errno = MPIR_Reduce_local((char *)tmpbuf, (char *)resbuf, count, datatype, op);
            MPIR_ERR_CHECK(mpi_errno);
            new_rank = vrank >> 1;
        }
    } else {
    ...
    if (!loop_flag) {
        while (mask < adjsize) {
            ...
            abs_partner = (partner < extra_ranks) ? (partner * 2) : (partner + extra_ranks);
            /* Compute the absolute rank */
            abs_partner = MPII_Bine_mod(abs_partner + root, comm_size);

            /* Compute send block boundaries inline */
            send_block_first = MPII_Bine_remap_rank(adjsize, partner) & block_first_mask;
